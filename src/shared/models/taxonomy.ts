...
import { shouldExcludeArchivedRecords } from '@/shared/lib/admin-content';
  ...
  const [result] = await db()
    ...
    .where(
      and(
        ...
        shouldExcludeArchivedRecords(status, includeArchived)
          ? ne(taxonomy.status, TaxonomyStatus.ARCHIVED)
          : undefined
      )
    )
    .limit(1);
