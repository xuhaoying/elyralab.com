...
export async function updateOrderInTransaction({
  ...
  const result = await db().transaction(async (tx: any) => {
    ...
    // Gate paid flow first: only one transaction can flip the order to paid.
    if (updateOrder.status === OrderStatus.PAID) {
      ...
      if (!paidOrderResult) {
        console.log(
          `Order ${orderNo} already paid or not in CREATED/PENDING status, skipping side effects`
        );
        return result;
      }

      result.order = paidOrderResult;
    }
    ...
    // non-paid flow keeps normal order update behavior
    if (updateOrder.status !== OrderStatus.PAID) {
