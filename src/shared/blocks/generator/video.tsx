...
export function VideoGenerator({
  ...
  const pollTaskStatus = useCallback(
    async (id: string) => {
      try {
        if (
          ...
          resetTaskState();
          toast.error(t('errors.user_facing.timeout'));
          return true;
        }
        ...
        if (currentStatus === AITaskStatus.SUCCESS) {
          if (videoUrls.length === 0) {
            toast.error(t('errors.user_facing.no_result'));
          } else {
            setGeneratedVideos(
              videoUrls.map((url, index) => ({
                id: `${task.id}-${index}`,
        ...
        if (currentStatus === AITaskStatus.FAILED) {
          const errorMessage = formatAITaskErrorMessage({
            input: parsedResult,
            t: (key) => t(`errors.${key}`),
          });
    ...
    [fetchUserCredits, generationStartTime, resetTaskState, t]
  );
  ...
  const handleGenerate = async () => {
    ...
    if (isVideoToVideoMode && !referenceVideoUrl) {
      toast.error(t('errors.reference_video_required'));
      return;
    }
