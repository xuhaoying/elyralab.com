...
export function MusicGenerator({ className, srOnlyTitle }: SongGeneratorProps) {
    ...
    try {
      ...
      if (generationStartTime) {
        ...
        if (elapsedTime > 180000) {
          ...
          toast.error(t('generator.errors.user_facing.timeout'));
      ...
      const songs = Array.isArray(task?.songs) ? task.songs : [];
  ...
  }, [fetchUserCredits, generationStartTime, t]);
  ...
  const handleGenerate = async () => {
    ...
    if (!provider || !model) {
      toast.error(t('generator.errors.invalid_model_config'));
      return;
    }
