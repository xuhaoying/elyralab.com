...
export function Nav({ nav, className }: { nav: NavType; className?: string }) {
  ...
  const isItemActive = (item?: NavItem) =>
    isUrlActive(item?.url as string) ||
    !!item?.children?.some((child) => isUrlActive(child?.url as string));
        ...
        <SidebarMenu>
          {nav.items.map((item: NavItem | undefined) => (
            <Collapsible
              ...
              asChild
              defaultOpen={item?.is_expand || isItemActive(item)}
              ...
              <SidebarMenuItem>
                {item?.children ? (
                  item?.url ? (
                    <>
                      <SidebarMenuButton
                        ...
                        tooltip={item?.title}
                        className={`${
                          item?.is_active || isItemActive(item)
                  ...
                  <SidebarMenuButton
                    ...
                    tooltip={item?.title}
                    className={`${
                      item?.is_active || isUrlActive(item?.url as string)
                        ? 'bg-sidebar-accent text-sidebar-accent-foreground hover:bg-sidebar-accent/90 hover:text-sidebar-accent-foreground active:bg-sidebar-accent/90 active:text-sidebar-accent-foreground min-w-8 duration-200 ease-linear'
                        : ''
                    }`}
                  >
                      ...
                      {item.children?.map((subItem: NavItem) => (
                        ...
                        >
                          <SidebarMenuSubButton
                            asChild
                            className={`${
                              subItem.is_active || isSubItemActive(subItem.url as string)
                                ? 'bg-sidebar-accent text-sidebar-accent-foreground hover:bg-sidebar-accent/90 hover:text-sidebar-accent-foreground active:bg-sidebar-accent/90 active:text-sidebar-accent-foreground min-w-8 duration-200 ease-linear'
                                : ''
                            }`}
                          >
